...
// Game Variables
let enemies = ["👹 Orc", "🧙‍♂️ Wizard", "🐉 Dragon", "🦄 Unicorn"];
let maxEnemyHealth = 75;
let enemyAttackDamageToHero = 25;

// Player Variables
let adventurerHealth = 100;
...
GAME: while (gameRunning) {
  ...
  while (enemyHealth > 0) {
    console.log(chalk.cyan(`\n\t🙎 Adventurer's HP: ${adventurerHealth}`));
    ...
    if (options.ans === "1. Attack") {
      ...
      if (numHealthPotions > 0) {
        adventurerHealth += healthPotionHealAmount;
        numHealthPotions--;
        console.log(chalk.green(`> You drink a health potion, healing yourself for ${healthPotionHealAmount}.`));
        console.log(chalk.cyan(`> You now have ${adventurerHealth} HP.`));
  ...
  console.log(chalk.cyan(`# You have ${adventurerHealth} HP left. #`));
